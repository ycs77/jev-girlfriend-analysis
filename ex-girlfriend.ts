...
if (message === undefined || message.trim() === '') {
  ...
  const response = await client.systemOne({
    ...
    questions: {
      ...
      landmine: choice(
        '依據 `message` 與 `context`，選出回覆這則前女友訊息時最容易踩到的地雷：不能說的話、不能做的事。用常見台詞當錨點判斷：提到共同回憶（如「今天經過那間麵店，還是會想到以前」）或深夜情緒（如「如果那時候我們都沒有那麼脾氣硬」）的地雷是開啟曖昧或長篇回憶；低風險試探開場（如「最近好嗎？」）的地雷是逼問意圖或長篇傾訴；帶刺比較（如「看來沒有我之後，你過得越來越順遂了嘛～」）的地雷是回嗆或辯解；求助或資源訊息（如電腦壞了、Netflix 帳號）的地雷是幫過頭或冷嘲；交接與界線訊息（如物品交接、指定聯絡管道）的地雷是跨管道寒暄或爭辯界線。沒有明顯地雷時選「無」。',
        {
  ...
  const nextSteps: Record<ExGirlfriendSituation, ActionPlan> = {
    ...
    optional_contact: {
      direction: '自然簡短回，觀察她後續是否持續主動；不要把這句當成復合或見面的邀請。',
